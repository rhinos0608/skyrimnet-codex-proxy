"""Xiaomi OpenAI-compatible provider."""
...
async def call_xiaomi_direct(system_prompt: Optional[str], messages: list, model: str, max_tokens: int, **extra_params) -> str:
    ...
    base = proxy.XIAOMI_PLATFORM_URL if api_model in proxy._XIAOMI_PLATFORM_MODELS else proxy.XIAOMI_BASE_URL
    ...
    try:
        try:
            content = await proxy._with_retry(_do_call, operation=f"Xiaomi {api_model}", request_id=request_id)
